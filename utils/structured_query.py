...
def _generate_success_factor_query(self):
    ...
    # Generate multi-step query - Fixed syntax
...
result = {{'total_records': total_count, 'successful_records': success_count, 'success_factor_percentage': round(success_factor, 2)}}
result"""
...
def _execute_multi_step_query(self, pandas_query, safe_globals):
    ...
    try:
        ...
        for i, statement in enumerate(statements):
            ...
            # Only store if it's an assignment (contains = but not inside a dict/comparison)
            if '=' in statement and not statement.startswith('{') and not statement.startswith('result'):
                try:
                    var_name = statement.split('=')[0].strip()
                    safe_globals[var_name] = result
                    log_event("INFO", f"Stored intermediate result: {var_name} = {result}")
                except:
                    pass  # Skip if can't parse variable name
...
def _format_result(self, result):
    ...
    if isinstance(result, pd.DataFrame):
        ...
        log_event("SUCCESS", f"Query executed successfully, returned dict result")
        return result
    else:
